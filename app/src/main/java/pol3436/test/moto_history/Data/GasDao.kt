...
interface GasDao {
  ...
  */
    ...
    /*
        @Insert(onConflict = OnConflictStrategy.IGNORE)
        fun add_ChangeItem(vararg changeItem: Change_Item)

        @Insert(onConflict = OnConflictStrategy.ROLLBACK)
        fun add_DefaltDataItem(vararg DefaltDataItem: Defalt_Data_Item)
        @Insert(onConflict = OnConflictStrategy.IGNORE)
        fun add_GasInputList(vararg GasInputList: Gas_InputList)

        @Insert(onConflict = OnConflictStrategy.IGNORE)
        fun add_RadiusGasStation(vararg RadiusGasStation: RadiusGasStation)
        @Insert(onConflict = OnConflictStrategy.IGNORE)
        fun add_SevenDayPrice(vararg SevenDayPrice: SevenDayPrice)
        @Insert(onConflict = OnConflictStrategy.IGNORE)
         fun add_Sido(vararg sido: Sido)
        @Insert(onConflict = OnConflictStrategy.IGNORE)
         fun add_SigunCode(vararg SigunCode: Sigun_code)
        @Insert(onConflict = OnConflictStrategy.ROLLBACK)
        fun add_SigunGasStation(vararg sigunGasStation: SigunGasStation)
        @Insert(onConflict = OnConflictStrategy.ROLLBACK)
        fun add_SigunLowprice(vararg sigunLowPrice: SigunLowPrice)
  ...
  */
   ...
   */
    @Query("SELECT * FROM Defalt_Data ORDER BY rowid")//" DESC")
    fun select_DefaltDate(): LiveData<List<Defalt_Data>>
